...
function catsDesdePuesto(puesto: string): string[] {
  ...
  if (p.includes("ganader")) return ["Encargado de Ganadería", "Capataz de Ganadería", "Peón General", "Puestero"]
  if (p.includes("casero") || p.includes("puestero")) return ["Puestero", "Caseros", "Peón General"]
  if (p.includes("tambo")) return ["Tambero", "Encargado de Tambo"]
  if (p.includes("agric") || p.includes("maquin") || p.includes("tractor")) return ["Tractorista", "Capataz de Agricultura", "Peón General"]
  return []
}
...
export function criteriosDesdeBusqueda(b: BusquedaRow): BusquedaMatch {
  ...
  if (b.edad_minima != null || b.edad_maxima != null)
    reqs.push({ campo: "edad", nivel: "deseable", min: b.edad_minima ?? undefined, max: b.edad_maxima ?? undefined })
  if (b.hectareas_min != null) reqs.push({ campo: "ha", nivel: "deseable", min: b.hectareas_min })
  if (b.personal_a_cargo_min != null) reqs.push({ campo: "gente", nivel: "deseable", val: b.personal_a_cargo_min })
  const edu = mapEdu(b.nivel_educacion)
  if (edu != null) reqs.push({ campo: "educacion", nivel: "deseable", val: edu })
  if (b.tipos_ganaderia_req?.length) reqs.push({ campo: "ganaderia", nivel: "deseable", val: b.tipos_ganaderia_req })
