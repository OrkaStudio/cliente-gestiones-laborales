...
export const EDU_NIVELES = ["Primario", "Secundario", "Terciario/Técnico", "Universitario", "Posgrado"] as const
...
export function reqLabel(r: Requisito): string {
  switch (r.campo) {
    case "hab": return r.hab
    case "vehiculo": return "Vehículo propio"
    case "residir": return "Disponible para residir"
    case "licencia": return "Licencia de conducir"
    case "hijos": return r.val ? "Con hijos" : "Sin hijos"
    case "edad": return `Edad ${r.min ?? ""}–${r.max ?? ""}`
    case "ha": return `Superficie ≥ ${r.min?.toLocaleString("es-AR") ?? ""}${r.max ? `–${r.max.toLocaleString("es-AR")}` : ""} ha`
    case "educacion": return `Educación mín. ${EDU_NIVELES[r.val]}`
    case "gente": return `Gente a cargo ≥ ${r.val}`
    case "ganaderia": return `Ganadería: ${r.val.join(" / ")}`
    case "civil": return `Estado civil: ${r.val}`
  }
}
...
export function rankear(cands: CandidatoMatch[], b: BusquedaMatch): { c: CandidatoMatch; score: Score; tier: Tier }[] {
  return cands
    .map((c) => {
      const score = scoreOf(c, b)
      return { c, score, tier: (score.failOblig ? "red" : score.hasAmber ? "amber" : "green") as Tier }
    })
