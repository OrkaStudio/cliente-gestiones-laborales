...
// Lista liviana de candidatos para el modal de vincular pareja. Se trae UNA sola
// vez (cacheada, tag candidatos-list) y el filtrado + ranking se hacen client-side
// → tipeo instantáneo, sin un viaje al servidor por tecla.
const getCandidatosLite = unstable_cache(
  async (): Promise<ParejaCandidato[]> => {
    const supabase = createServiceClient()
    const { data } = await supabase
      .from("candidatos")
      .select("id, nombre, apellido, ultimo_puesto, ubicacion, estado_civil")
    return (data ?? []) as unknown as ParejaCandidato[]
  },
  ["candidatos-lite-pareja"],
  { tags: ["candidatos-list"], revalidate: 120 },
)
