 ...
 */
import { BUSQUEDA_DEMO, getCandidatosFixture } from "../src/lib/v2/fixture";
import { evalCat, evalReq, rankear } from "../src/lib/v2/matching";
...
for (const r of ranked.slice(0, 12)) {
  console.log(
    `  ${tierTxt[r.tier]} ${String(r.score.s).padStart(4)}  ${r.c.nombre} ${r.c.apellido}  ·  ${r.c.cats.join(", ") || "sin categoría"}`,
  );
}
...
for (const r of ranked.slice(0, 2)) {
  ...
  console.log(
    `  categoría (obligatorio): ${ce === "ok" ? "✓" : ce === "no" ? "✕" : "?"}  [${r.c.cats.join(", ") || "—"}]`,
  );
