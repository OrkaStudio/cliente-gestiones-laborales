...
async function main() {
  ...
  for (let i = 0; i < total; i++) {
    ...
    try {
      ...
      console.error(
        `  ✗ ${nombre}: ${err instanceof Error ? err.message : String(err)} — reintento en 5s`,
      );
      await sleep(5000);
      try {
        r = await extraerHabilidadesYResidir(c.cv_procesado_texto ?? "");
        ...
        console.error(
          `  ✗✗ ${nombre}: falló de nuevo, salteo. ${err2 instanceof Error ? err2.message : String(err2)}`,
        );
        continue;
      }
    }
    ...
    console.log(
      `        habilidades (${r.habilidades.length}): ${r.habilidades.join(", ") || "—"}`,
    );
  ...
  console.log("\n──────── Resumen ────────");
  console.log(
    `residir → si: ${resumen.si} · no: ${resumen.no} · sin_dato: ${resumen.sin_dato} · (con zona: ${resumen.conZona})`,
  );
